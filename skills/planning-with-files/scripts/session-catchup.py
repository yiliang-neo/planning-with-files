...
PLANNING_FILE_PREFIXES = ["task_plan_", "progress_", "findings_"]
PLANNING_SUBDIR = ".idea"
...
def planning_file_from_path(path_value: Any) -> Optional[str]:
    ...
    for prefix in PLANNING_FILE_PREFIXES:
...
def planning_file_from_paths(paths: Iterable[Any]) -> Optional[str]:
    for path in paths:
        result = planning_file_from_path(path)
        if result:
            return result
    return None
...
def main():
    ...
    # Check if planning files exist in .idea/ subdir (indicates active task)
    idea_dir = Path(project_path) / PLANNING_SUBDIR
    has_planning_files = idea_dir.is_dir() and any(
        f.name.startswith(prefix) and f.name.endswith(".md")
        for f in idea_dir.iterdir()
        for prefix in PLANNING_FILE_PREFIXES
    )
    ...
    print("2. Read: .idea/task_plan_*.md, .idea/progress_*.md, .idea/findings_*.md")
